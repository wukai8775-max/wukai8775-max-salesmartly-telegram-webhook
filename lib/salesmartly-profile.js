...
function normalizeOfficialPayload(payload = {}) {
  ...
  const senderType = firstNonEmpty(findDeepValue(payload, ["sender_type"]), data.sender_type);
  const sysUserId = firstNonEmpty(findDeepValue(payload, ["sys_user_id"]), data.sys_user_id);

  const normalized = {
    event_type: firstNonEmpty(findDeepValue(payload, ["event", "event_type", "eventName", "type", "action"])),
    ws_display_name: getDisplayNameFromPayload(payload),
    customer_name: firstNonEmpty(findDeepValue(payload, ["customer_name", "name", "remark_name"])),
    phone: firstNonEmpty(findDeepValue(payload, ["phone", "phone_number", "customer_phone", "mobile", "tel"])),
    email: firstNonEmpty(findDeepValue(payload, ["email", "customer_email"])),
    channel: firstNonEmpty(findDeepValue(payload, ["channel", "channel_name", "platform", "source"])),
    channel_id: firstNonEmpty(findDeepValue(payload, ["channel_id"])),
    channel_uid: firstNonEmpty(findDeepValue(payload, ["channel_uid"])),
    contact_id: firstNonEmpty(findDeepValue(payload, ["contact_id", "chat_user_id", "customer_id", "user_id"])),
    session_id: firstNonEmpty(findDeepValue(payload, ["session_id", "chat_session_id", "conversation_id"])),
    project_id: firstNonEmpty(findDeepValue(payload, ["project_id"])),
    conversation_url: firstNonEmpty(findDeepValue(payload, ["conversation_url", "chat_url", "session_url", "url"])),
    last_message: lastMessage,
    message_time: messageTime,
    sender_name: firstNonEmpty(findDeepValue(payload, ["sender_name", "sender", "sys_user_name", "staff_name"])),
