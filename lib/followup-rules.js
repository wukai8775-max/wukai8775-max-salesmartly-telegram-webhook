...
function classifyCustomerStatus(messages = [], customer = {}) {
  ...
  if (
    latestCustomerAt &&
    latestAgentAt &&
    latestAgentAt.getTime() > latestCustomerAt.getTime() &&
    isSimpleGreetingMessage(latestCustomerText)
  ) {
...
function getNextFollowupStage({ customer = {}, messages = [], logs = [], tasks = [], now = new Date(), force = false }) {
  ...
  for (const item of FOLLOWUP_STAGES) {
    ...
    if ((force || now.getTime() >= dueAt.getTime()) && !stageAlreadySent(item.stage, logs, tasks)) {
      return {
        due: true,
        stage: item.stage,
  ...
  if (finalReminderSent) {
    return { due: false, should_stop: true, stop_reason: "no_reply_after_3_days" };
  }

  return { due: false, skipped_reason: "no_due_stage" };
}
...
function analyzeFollowupDecision({ customer = {}, messages = [], logs = [], tasks = [], now = new Date(), force = false }) {
  ...
  if (!LOW_RISK_REMINDER_STATUSES.has(status)) {
    return {
      ...
      auto_send_allowed: false,
      telegram_alert_allowed: false,
      skipped_reason: "status_not_reminder_allowed",
    };
  }

  if (!customerHasUnansweredAgentMessage(customer, messages)) {
    return {
      status,
      risk_level: riskLevel,
      auto_send_allowed: false,
