...
function classifyCustomerStatus(messages = [], customer = {}) {
  ...
  if (
    hasPattern(latestCustomerText || recentText, [
      /\bprice\s+(is|are|seems?|looks?)\s+(very\s+)?high\b/i,
      /\bprices\s+(are|seem|look)\s+(very\s+)?high\b/i,
  ...
  if (
    hasPattern(latestCustomerText || recentText, [
      ...
      /\bmenu\b/i,
      /\bproduct\s+list\b/i,
      /\bcan\s+i\s+see\s+your\s+catalog(?:ue)?\b/i,
      /\bcan\s+i\s+have\s+a\s+price\s+list\b/i,
    ])
  ) {
  ...
  if (
    hasPattern(latestCustomerText || recentText, [
      /\bdo\s+you\s+carry\b/i,
      /\bdo\s+you\s+have\b/i,
      /\bpills?\b/i,
      /\boils?\b/i,
      /\bproduct\s+question\b/i,
      /\bwhat\s+products?\b/i,
      /\bwhich\s+products?\b/i,
    ])
  ) {
...
function getNextFollowupStage({
  ...
  if (!latestAgentAt) {
    return {
      due: false,
      skipped_reason: "missing_staff_message_before_followup",
    };
  }
  ...
  if (latestCustomerAt && latestAgentAt.getTime() <= latestCustomerAt.getTime()) {
    return {
      due: false,
      skipped_reason: "customer_replied_after_staff_message",
    };
  }

  const referenceAt = latestAgentAt;
  const stages = FOLLOWUP_STAGES;
