...
const DEFAULT_STAFF_ID_NAME_MAP = {
  "1199730": "Omen",
  "1199741": "Omen Agent",
  "1203624": "Jett Agent",
};
const DEFAULT_FOLLOWUP_REMINDER_STAFF_ID_ALLOWLIST = ["1199730", "1199741", "1203624"];

let cachedStaffIdNameMapRaw;
let cachedStaffIdNameMap = { ...DEFAULT_STAFF_ID_NAME_MAP };
let cachedFollowupStaffAllowlistRaw;
let cachedFollowupStaffAllowlist = DEFAULT_FOLLOWUP_REMINDER_STAFF_ID_ALLOWLIST;
...
function isUnknownValue(value) {
  ...
  return !text || text === UNKNOWN_STAFF_NAME || text === UNKNOWN_STAFF_ID;
}

function parseStaffIdNameMap() {
  const raw = process.env.STAFF_ID_NAME_MAP || "";

  if (raw === cachedStaffIdNameMapRaw) {
    return cachedStaffIdNameMap;
  }

  cachedStaffIdNameMapRaw = raw;
  cachedStaffIdNameMap = { ...DEFAULT_STAFF_ID_NAME_MAP };
  ...
  try {
    ...
    if (!isPlainObject(parsed)) {
      throw new Error("STAFF_ID_NAME_MAP must be a JSON object");
    }

    cachedStaffIdNameMap = {
      ...DEFAULT_STAFF_ID_NAME_MAP,
      ...Object.fromEntries(
        Object.entries(parsed)
          .map(([id, name]) => [String(id).trim(), firstNonEmpty(name)])
          .filter(([id, name]) => id && name)
      ),
    };
  } catch (error) {
    console.warn("Invalid STAFF_ID_NAME_MAP", {
      staff_id_name_map_exists: true,
      error_name: error?.name || "Error",
      error_message: String(error?.message || "invalid_json").slice(0, 120),
    });
    cachedStaffIdNameMap = { ...DEFAULT_STAFF_ID_NAME_MAP };
  }
...
function getStaffNameFromIdMap(staffId) {
  const id = firstNonEmpty(staffId);
  if (!id || id === UNKNOWN_STAFF_ID) {
    return "";
  }

  return firstNonEmpty(parseStaffIdNameMap()[String(id).trim()]);
}
...
function getAssignedStaffProfile(customer = {}, messages = []) {
  ...
  const resolvedName = resolveCandidate(
    [
      ...
      ["env.STAFF_ID_NAME_MAP", getStaffNameFromIdMap(resolvedId.value)],
    ],
    UNKNOWN_STAFF_NAME,
    "unresolved"
  );
...
function getFollowupStaffAllowlistState() {
  const raw = process.env.FOLLOWUP_REMINDER_STAFF_ID_ALLOWLIST || DEFAULT_FOLLOWUP_REMINDER_STAFF_ID_ALLOWLIST.join(",");

  if (raw === cachedFollowupStaffAllowlistRaw) {
    return {
      enabled: cachedFollowupStaffAllowlist.length > 0,
      ids: cachedFollowupStaffAllowlist,
    };
  }

  cachedFollowupStaffAllowlistRaw = raw;
  cachedFollowupStaffAllowlist = [...new Set(
    String(raw || "")
      .split(/[,;\s]+/)
...
function isStaffInFollowupAllowlist(staffId, allowlistState = getFollowupStaffAllowlistState()) {
  ...
  return allowlistState.ids.includes(String(id).trim());
}
...
module.exports = {
  UNKNOWN_STAFF_NAME,
  UNKNOWN_STAFF_ID,
  DEFAULT_FOLLOWUP_REMINDER_STAFF_ID_ALLOWLIST,
  getStaffNameFromPayload,
  getStaffIdFromPayload,
  getStaffNameFromIdMap,
