...
const { getStaffNameFromPayload, getStaffIdFromPayload } = require("./staff-profile");
...
function logOfficialWebhookStorage(payload = {}, profile = {}, direction = "system", result = {}) {
  ...
  console.info("SaleSmartly official webhook Supabase storage", {
    ...
    extracted_staff_name_exists: Boolean(firstNonEmpty(profile.assigned_staff_name, profile.assigned_ai_employee, profile.last_agent_sender_name, getStaffNameFromPayload(payload))),
...
function isMissingOptionalCustomerColumnError(error) {
  ...
  return (
    ...
    OPTIONAL_CUSTOMER_COLUMNS.some((column) => message.includes(column))
  );
}
...
function buildCustomerRow(profile = {}, options = {}) {
  ...
  const agentSenderName =
    direction === "ai" || direction === "human"
      ...
      : "";
  const assignedStaffName = firstNonEmpty(profile.assigned_staff_name, payloadStaffName, agentSenderName);
  const row = {
    ...
    assigned_staff_id: firstNonEmpty(profile.assigned_staff_id, payloadStaffId),
...
async function upsertCustomer(profile = {}, options = {}) {
  ...
  const row = buildCustomerRow(profile, options);
  const direction = firstNonEmpty(options.direction, "system");
  const messageTime = normalizeMessageTime(options.messageTime || profile.message_time);
