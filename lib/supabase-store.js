...
function encodeInValues(values = []) {
  return values.map((value) => encodeFilter(value)).join(",");
}
...
function getActiveFollowupCustomerFilters() {
  return [
    "followup_stopped=not.is.true",
    "do_not_followup=not.is.true",
    "last_agent_message_at=not.is.null",
    "last_customer_message_at=not.is.null",
  ];
}

function getCustomerIdentityKey(customer = {}) {
  return firstNonEmpty(customer.contact_id && `contact:${customer.contact_id}`, customer.session_id && `session:${customer.session_id}`);
}

async function selectCustomersByStaffAllowlist(limit, offset, staffIds = []) {
  ...
  return selectRows(
    ...
    [
      "select=*",
      `assigned_staff_id=in.(${encodeInValues(staffIds)})`,
      ...getActiveFollowupCustomerFilters(),
...
async function getAllowedStaffMessageRefs(staffIds = [], limit = 200) {
  if (!staffIds.length) {
    return [];
  }
  ...
  return selectRows(
    "messages",
    [
      "select=contact_id,session_id,sender_id,message_time",
      "direction=in.(ai,human)",
      `sender_id=in.(${encodeInValues(staffIds)})`,
...
async function selectCustomersByIdentityField(field, values = [], limit = 100) {
  const ids = uniqueNonEmpty(values).slice(0, 100);
  if (!ids.length) {
    return [];
  }
...
async function selectCustomersByAllowedStaffMessages(limit, existingCustomers = [], staffIds = []) {
  if (!staffIds.length || existingCustomers.length >= limit) {
    return [];
  }

  const refs = await getAllowedStaffMessageRefs(staffIds, Math.min(Math.max(limit * 6, 60), 300));
  const existingKeys = new Set(existingCustomers.map(getCustomerIdentityKey).filter(Boolean));
  const contactIds = [];
  const sessionIds = [];

  for (const ref of refs) {
    if (ref.contact_id && !existingKeys.has(`contact:${ref.contact_id}`)) {
      contactIds.push(ref.contact_id);
    } else if (ref.session_id && !existingKeys.has(`session:${ref.session_id}`)) {
      sessionIds.push(ref.session_id);
    }
  }

  const remaining = Math.max(limit - existingCustomers.length, 0);
  const byContact = await selectCustomersByIdentityField("contact_id", contactIds, remaining);
  const merged = [...byContact];

  if (merged.length < remaining) {
    const bySession = await selectCustomersByIdentityField("session_id", sessionIds, remaining - merged.length);
    merged.push(...bySession);
  }
...
async function selectKnownHighRiskCustomers(limit, existingCustomers = []) {
  const safeLimit = Number.isFinite(Number(limit)) && Number(limit) > 0 ? Math.min(Number(limit), 20) : 20;
  const existingKeys = new Set(existingCustomers.map(getCustomerIdentityKey).filter(Boolean));
  ...
  const statusRows = await selectRows(
    ...
    [
      "select=*",
      `current_status=in.(${encodeInValues(HIGH_RISK_STATUSES)})`,
      "followup_stopped=not.is.true",
      "order=updated_at.asc",
      `limit=${encodeURIComponent(String(safeLimit))}`,
    ].join("&")
  ...
  return [...riskRows, ...statusRows].filter((customer) => {
    const key = getCustomerIdentityKey(customer);
    if (!key || seen.has(key)) {
      return false;
    }
...
async function listCustomersForAnalysis(limit = 50, options = {}) {
  const safeLimit = Number.isFinite(Number(limit)) && Number(limit) > 0 ? Number(limit) : 50;
  const offset = Number.isFinite(Number(options.offset)) && Number(options.offset) > 0 ? Number(options.offset) : 0;
  const staffIds = uniqueNonEmpty(options.staffAllowlistIds || []);

  if (options.filterByStaffAllowlist && staffIds.length) {
    const assignedStaffCustomers = await selectCustomersByStaffAllowlist(safeLimit, offset, staffIds);
    const messageStaffCustomers = await selectCustomersByAllowedStaffMessages(safeLimit, assignedStaffCustomers, staffIds);
    const highRiskCustomers = await selectKnownHighRiskCustomers(20, [...assignedStaffCustomers, ...messageStaffCustomers]);
    const customers = [...assignedStaffCustomers, ...messageStaffCustomers, ...highRiskCustomers].slice(0, safeLimit);

    return {
      customers,
      query_filtered_by_staff_allowlist: true,
      candidate_count_after_staff_filter: customers.length,
      skipped_before_processing_staff_not_allowlisted: 0,
      assigned_staff_candidate_count: assignedStaffCustomers.length,
      message_staff_candidate_count: messageStaffCustomers.length,
      high_risk_candidate_count: highRiskCustomers.length,
    };
  }
