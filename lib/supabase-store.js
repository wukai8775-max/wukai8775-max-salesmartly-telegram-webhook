...
async function findCustomer(profile = {}) {
  ...
  if (contactId) {
    const rows = await selectRows(
      "customers",
      `select=*&contact_id=eq.${encodeFilter(contactId)}&limit=1`
    );
    if (rows[0]) {
      return rows[0];
    }
  ...
  if (sessionId) {
    const rows = await selectRows(
      "customers",
      `select=*&session_id=eq.${encodeFilter(sessionId)}&limit=1`
    );
    if (rows[0]) {
      return rows[0];
    }
...
function buildCustomerRow(profile = {}, options = {}) {
  ...
  const row = {
    ...
    assigned_ai_employee: firstNonEmpty(
...
async function upsertCustomer(profile = {}, options = {}) {
  ...
  return upsertRow("customers", row, conflictKey);
}
...
async function insertMessage(message = {}) {
  ...
  const row = {
    ...
    direction: firstNonEmpty(message.direction, "system"),
    sender_name: firstNonEmpty(message.sender_name),
...
async function saveOfficialWebhookEvent(profile = {}, payload = {}, direction = "system") {
  ...
  if (willUpsertCustomer) {
    try {
      customer = await upsertCustomer(profile, {
        direction,
        messageText,
        messageTime,
      });
  ...
  const result = {
    saved: Boolean(customer || message),
    will_upsert_customer: willUpsertCustomer,
    will_insert_message: willInsertMessage,
