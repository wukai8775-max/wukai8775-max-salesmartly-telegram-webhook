...
Use an external HTTP scheduler to call once per hour. For cron-job.org, keep each request small because the platform times out at about 30 seconds:
...
For cron-job.org, recommended setup is either one small job first:

```text
/api/cron-analyze-followups?secret=<CRON_SECRET>&limit=50&offset=0
```

or multiple sharded jobs:
...
The cron endpoint is stateless. It does not require a new database cursor table. Use `next_offset` / `next_cursor` from the response as the next `offset` if you want an external scheduler to rotate through pages. When the endpoint reaches the end of the customer list, `next_offset` resets to `0`.

When `FOLLOWUP_REMINDER_STAFF_ID_ALLOWLIST` is configured, cron filtering is pushed down into Supabase as much as possible:

```text
customers.assigned_staff_id in allowlist
followup_stopped is not true
do_not_followup is not true
last_agent_message_at is not null
...
- defaults to `limit=50` and caps at `limit=100`;
- uses `cursor` as an alias for `offset`;
- treats `page` as 1-based, so `page=2&limit=50` equals `offset=50`;
- stops before cron-job.org's 30 second timeout, normally around the 20 second mark;
...
```json
{
  ...
  "processed_count": 50,
  "elapsed_ms": 12840,
  "stopped_reason": "more_pages_available",
  "scanned": 50,
